...
packageDeclaration <-  packageModifier* PACKAGE Identifier^Err_003 ('.' Identifier^Err_004)* ';'^Err_005
packageModifier <-  annotation
importDeclaration <-  IMPORT STATIC? qualIdent^Err_006 ('.' '*'^Err_007)? ';'^Err_008  /  ';'
typeDeclaration <-  classDeclaration  /  interfaceDeclaration  /  ';'
classDeclaration <-  normalClassDeclaration  /  enumDeclaration
normalClassDeclaration <-  classModifier* CLASS Identifier^Err_009 typeParameters? superclass? superinterfaces? classBody^Err_010
...
formalParameter <-  variableModifier* unannType variableDeclaratorId  /  variableModifier* unannType annotation* '...' variableDeclaratorId^Err_012 !','
...
throws          <-  THROWS exceptionTypeList^Err_013
exceptionTypeList <-  exceptionType^Err_014 (',' exceptionType^Err_015)*
...
normalInterfaceDeclaration <-  interfaceModifier* INTERFACE Identifier^Err_020 typeParameters? extendsInterfaces? interfaceBody^Err_021
interfaceModifier <-  annotation  /  PUBLIC  /  PROTECTED  /  PRIVATE  /  ABSTRACT  /  STATIC  /  STRICTFP
extendsInterfaces <-  EXTENDS interfaceTypeList
interfaceBody   <-  '{'^Err_022 interfaceMemberDeclaration* '}'^Err_023
...
annotationTypeDeclaration <-  interfaceModifier* '@' INTERFACE Identifier^Err_024 annotationTypeBody^Err_025
annotationTypeBody <-  '{'^Err_026 annotationTypeMemberDeclaration* '}'^Err_027
...
statement       <-  block  /  IF parExpression^Err_028 statement^Err_029 (ELSE statement^Err_030)?  /  basicForStatement  /  enhancedForStatement  /  WHILE parExpression statement  /  DO statement^Err_031 WHILE^Err_032 parExpression^Err_033 ';'^Err_034  /  tryStatement  /  SWITCH parExpression^Err_035 switchBlock^Err_036  /  SYNCHRONIZED parExpression block  /  RETURN expression? ';'^Err_037  /  THROW expression^Err_038 ';'^Err_039  /  BREAK Identifier? ';'^Err_040  /  CONTINUE Identifier? ';'^Err_041  /  ASSERT expression^Err_042 (':' expression^Err_043)? ';'^Err_044  /  ';'  /  statementExpression ';'  /  Identifier ':' statement
statementExpression <-  assignment  /  ('++'  /  '--') (primary  /  qualIdent)  /  (primary  /  qualIdent) ('++'  /  '--')  /  primary
switchBlock     <-  '{'^Err_045 switchBlockStatementGroup* switchLabel* '}'^Err_046
switchBlockStatementGroup <-  switchLabels blockStatements
switchLabels    <-  switchLabel switchLabel*
switchLabel     <-  CASE (constantExpression  /  enumConstantName^Err_047)^Err_048 ':'^Err_049  /  DEFAULT ':'
enumConstantName <-  Identifier^Err_050
...
tryStatement    <-  TRY (block (catchClause* finally  /  catchClause+)  /  resourceSpecification^Err_051 block^Err_052 catchClause* finally?)^Err_053
catchClause     <-  CATCH '('^Err_054 catchFormalParameter^Err_055 ')'^Err_056 block^Err_057
catchFormalParameter <-  variableModifier* catchType^Err_058 variableDeclaratorId^Err_059
catchType       <-  unannClassType ('|' ![=|] classType)*
finally         <-  FINALLY block^Err_060
resourceSpecification <-  '('^Err_061 resourceList^Err_062 ';'? ')'^Err_063
resourceList    <-  resource^Err_064 (';' resource)*
resource        <-  variableModifier* unannType variableDeclaratorId '=' !'=' expression
expression      <-  lambdaExpression  /  assignmentExpression
primary         <-  primaryBase primaryRest*
primaryBase     <-  THIS  /  Literal  /  parExpression  /  SUPER ('.' typeArguments? Identifier arguments  /  '.' Identifier  /  '::' typeArguments? Identifier)  /  NEW (classCreator  /  arrayCreator)  /  qualIdent ('[' expression ']'  /  arguments  /  '.' (THIS  /  NEW classCreator  /  typeArguments Identifier arguments  /  SUPER '.' typeArguments? Identifier arguments  /  SUPER '.' Identifier^Err_065  /  SUPER '::' typeArguments? Identifier^Err_066 arguments^Err_067)  /  ('[' ']')* '.' CLASS  /  '::' typeArguments? Identifier)  /  VOID '.' CLASS^Err_068  /  basicType ('[' ']')* '.' CLASS  /  referenceType '::' typeArguments? NEW  /  arrayType '::' NEW
...
unaryExpressionNotPlusMinus <-  '~' unaryExpression^Err_069  /  '!' ![=&] unaryExpression^Err_070  /  castExpression  /  (primary  /  qualIdent) ('++'  /  '--')?
castExpression  <-  '(' primitiveType ')' unaryExpression^Err_071  /  '(' referenceType additionalBound* ')' lambdaExpression  /  '(' referenceType additionalBound* ')' unaryExpressionNotPlusMinus
infixExpression <-  unaryExpression (infixOperator unaryExpression  /  INSTANCEOF referenceType^Err_072)*
infixOperator   <-  '||'  /  '&&'  /  '|' ![=|]  /  '^' ![=]  /  '&' ![=&]  /  '=='  /  '!='  /  '<' ![=<]  /  '>' ![=>]  /  '<='  /  '>='  /  '<<' ![=]  /  '>>' ![=>]  /  '>>>' ![=]  /  '+' ![=+]  /  '-' ![-=>]  /  '*' ![=]  /  '/' ![=]  /  '%' ![=]
conditionalExpression <-  infixExpression ('?' expression ':' expression)*
assignmentExpression <-  assignment  /  conditionalExpression
assignment      <-  leftHandSide AssignmentOperator expression^Err_073
leftHandSide    <-  primary  /  qualIdent
AssignmentOperator <-  '=' ![=]  /  '*='  /  '/='  /  '%='  /  '+='  /  '-='  /  '<<='  /  '>>='  /  '>>>='  /  '&='  /  '^='  /  '|='
lambdaExpression <-  lambdaParameters '->' lambdaBody^Err_074
...
lambdaBody      <-  (expression  /  block^Err_075)^Err_076
