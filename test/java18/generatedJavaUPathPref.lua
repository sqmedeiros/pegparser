...
statement       <-  block  /  'if' parExpression^Err_034 statement^Err_035 ('else' statement^Err_036)?  /  basicForStatement  /  enhancedForStatement  /  'while' parExpression statement  /  'do' statement^Err_037 'while'^Err_038 parExpression^Err_039 ';'^Err_040  /  tryStatement  /  'switch' parExpression^Err_041 switchBlock^Err_042  /  'synchronized' parExpression block  /  'return' expression? ';'^Err_043  /  'throw' expression^Err_044 ';'^Err_045  /  'break' Identifier? ';'^Err_046  /  'continue' Identifier? ';'^Err_047  /  'assert' expression^Err_048 (':' expression^Err_049)? ';'^Err_050  /  ';'  /  statementExpression ';'  /  Identifier ':' statement
statementExpression <-  assignment  /  ('++'  /  '--') (primary  /  qualIdent)  /  (primary  /  qualIdent) ('++'  /  '--')  /  primary
switchBlock     <-  '{'^Err_051 switchBlockStatementGroup* switchLabel* '}'^Err_052
switchBlockStatementGroup <-  switchLabels blockStatements
switchLabels    <-  switchLabel switchLabel*
switchLabel     <-  'case' (constantExpression  /  enumConstantName)^Err_053 ':'^Err_054  /  'default' ':'
enumConstantName <-  Identifier^Err_055
...
tryStatement    <-  'try' (block (catchClause* finally  /  catchClause+)^Err_056  /  resourceSpecification block^Err_057 catchClause* finally?)^Err_058
catchClause     <-  'catch' '('^Err_059 catchFormalParameter^Err_060 ')'^Err_061 block^Err_062
catchFormalParameter <-  variableModifier* catchType^Err_063 variableDeclaratorId^Err_064
catchType       <-  unannClassType^Err_065 ('|' ![=|] classType^Err_066)*
finally         <-  'finally' block^Err_067
resourceSpecification <-  '('^Err_068 resourceList^Err_069 ';'? ')'^Err_070
resourceList    <-  resource^Err_071 (',' resource^Err_072)*
resource        <-  variableModifier* unannType^Err_073 variableDeclaratorId^Err_074 '='^Err_075 !'=' expression^Err_076
expression      <-  lambdaExpression  /  assignmentExpression
primary         <-  primaryBase primaryRest*
primaryBase     <-  'this'  /  Literal  /  parExpression  /  'super' ('.' typeArguments? Identifier arguments  /  '.' Identifier  /  '::' typeArguments? Identifier)  /  'new' (classCreator  /  arrayCreator)^Err_077  /  qualIdent ('[' expression ']'  /  arguments  /  '.' ('this'  /  'new' classCreator  /  typeArguments Identifier arguments  /  'super' '.' typeArguments? Identifier arguments  /  'super' '.' Identifier  /  'super' '::' typeArguments? Identifier arguments)  /  ('[' ']')* '.' 'class'  /  '::' typeArguments? Identifier)  /  'void' '.' 'class'^Err_078  /  basicType ('[' ']')* '.' 'class'  /  referenceType '::' typeArguments? 'new'  /  arrayType '::' 'new'
...
arrayCreator    <-  (type dimExpr+ dim*  /  type dim+^Err_079 arrayInitializer^Err_080)^Err_081
dimExpr         <-  annotation* '[' expression ']'
arguments       <-  '(' argumentList? ')'
argumentList    <-  expression (',' expression)*
unaryExpression <-  ('++'  /  '--') (primary  /  qualIdent)  /  '+' ![=+] unaryExpression^Err_082  /  '-' ![-=>] unaryExpression^Err_083  /  unaryExpressionNotPlusMinus
unaryExpressionNotPlusMinus <-  '~' unaryExpression^Err_084  /  '!' ![=&] unaryExpression^Err_085  /  castExpression  /  (primary  /  qualIdent) ('++'  /  '--')?
castExpression  <-  '(' primitiveType ')' unaryExpression^Err_086  /  '(' referenceType additionalBound* ')' lambdaExpression  /  '(' referenceType additionalBound* ')' unaryExpressionNotPlusMinus
infixExpression <-  unaryExpression (InfixOperator unaryExpression^Err_087  /  'instanceof' referenceType^Err_088)*
InfixOperator   <-  '||'  /  '&&'  /  '|' ![=|]  /  '^' ![=]  /  '&' ![=&]  /  '=='  /  '!='  /  '<' ![=<]  /  '>' ![=>]  /  '<='  /  '>='  /  '<<' ![=]  /  '>>' ![=>]  /  '>>>' ![=]  /  '+' ![=+]  /  '-' ![-=>]  /  '*' ![=]  /  '/' ![=]  /  '%' ![=]
conditionalExpression <-  infixExpression ('query' expression^Err_089 ':'^Err_090 expression^Err_091)*
assignmentExpression <-  assignment  /  conditionalExpression
assignment      <-  leftHandSide AssignmentOperator expression^Err_092
leftHandSide    <-  primary  /  qualIdent
AssignmentOperator <-  '=' ![=]  /  '*='  /  '/='  /  '%='  /  '+='  /  '-='  /  '<<='  /  '>>='  /  '>>>='  /  '&='  /  '^='  /  '|='
lambdaExpression <-  lambdaParameters '->' lambdaBody^Err_093
...
lambdaBody      <-  (expression  /  block)^Err_094
